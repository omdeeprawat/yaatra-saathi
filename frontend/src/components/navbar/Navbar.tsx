...
export default function Navbar() {
  ...
  if (user?.role === "admin") {
    privateLinks.push({ to: "/admin", label: "Admin" });
  }
    ...
    <nav
      className={`sticky top-0 z-50 border-b backdrop-blur-xl transition-colors duration-300 ${
        isLight
          ...
          : "border-mountain-600/65 bg-mountain-900/72"
      }`}
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <Link to="/" className="flex items-center gap-2 group">
            <div className="w-9 h-9 rounded-lg overflow-hidden bg-white/95 ring-1 ring-saffron-500/60 shadow-[0_0_18px_rgba(14,165,233,0.28)] shrink-0">
              <img
                src={projectLogo}
              ...
              <NavLink
                ...
                className={({ isActive }) =>
                  clsx(
                    ...
                    isActive &&
                      (isLight ? "text-saffron-600" : "text-saffron-300 drop-shadow-[0_0_10px_rgba(56,189,248,0.5)]"),
                  )
                }
              >
                {link.label}
            ...
            {isAuthenticated ? (
              <div className="relative" ref={dropdownRef}>
                <button
                  ...
                  className="flex items-center gap-2 rounded-full p-0.5 hover:ring-2 hover:ring-saffron-500/60 hover:shadow-[0_0_20px_rgba(14,165,233,0.25)] transition-all focus:outline-none focus:ring-2 focus:ring-saffron-500"
                >
                  {user?.avatar_url ? (
                    <img
                      src={user.avatar_url}
                  ...
                  <div
                    className={`absolute right-0 mt-2 w-56 rounded-xl border shadow-xl py-2 z-50 transition-colors duration-300 ${
                      isLight
                        ...
                        : "bg-mountain-800/96 border-mountain-600 shadow-black/40"
                    }`}
                  >
                    {/* User info header */}
                    <div
        ...
        <div
          className={`md:hidden border-t px-4 py-4 flex flex-col gap-4 transition-colors duration-300 ${
            isLight
              ...
              : "border-mountain-600/55 bg-mountain-900/95"
          }`}
        >
          {links.map((link) => (
            <NavLink
